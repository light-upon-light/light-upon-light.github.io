...
paragraphs, before the first `##` (quran.md is the exception: see above). When it follows the `details.tldr` card (quran.md), wrap both in `<div class="intro-toggles" markdown="1">` so the closed pill stays centred with the key-terms box beside it on the left; below 48em the two become a centred pair on one row. It replaces the old hand-written
